...
#
# These reproduce a *genuine* Sony DualSense (CFI-ZCT1) USB identity, based on a
# descriptor capture from physical hardware and cross-checked against DS5_Bridge.
# Games use this identity to locate the controller's separate four-channel haptic
# render stream without replacing the main game-audio render stream.
#
#   * Genuine 273-byte USB HID report descriptor.  Do not add feature reports that
#     are absent from the physical controller: strict titles inspect this descriptor.
...
#   * Haptic OUT endpoint is Isochronous *Adaptive* (0x09), with bInterval=4.
#     At high speed that is one 1-ms service interval; advertising bInterval=1
#     incorrectly asks Windows for 125-us service and distorts the PCM timing.
...
#
# USB/IP speed value 3 is USB 2.0 High Speed (480 Mbps), as negotiated by a
# physical DualSense.  At high speed bInterval=4 represents a 1-ms interval.
DUALSENSE_USBIP_SPEED = 3
DUALSENSE_MIC_BYTES_PER_INTERVAL = 192  # 2ch * 16-bit * 48kHz * 1ms
...
#      IT4 (Headset Mic, 2-ch)   -> FU5 -> OT6 (USB streaming)
#  - Interface 1 (alt 0/1): Audio Streaming OUT (4-ch/16-bit/48kHz, iso EP 0x01, Adaptive)
#  - Interface 2 (alt 0/1): Audio Streaming IN  (2-ch/16-bit/48kHz, iso EP 0x82, Async)
#  - Interface 3:          HID (DualSense, EP 0x84 IN, EP 0x03 OUT)
# Byte-faithful to a physical CFI-ZCT1 configuration descriptor.
DUALSENSE_CONFIGURATION_DESCRIPTOR = bytes.fromhex(
    ...
    # Input Terminal 4: Headset Mic, 2 channels
    "0C 24 02 04 02 04 03 02 03 00 00 00"
    ...
    # Endpoint 0x01 OUT: Isochronous Adaptive, wMaxPacketSize 392 (0x0188), bInterval 4
    "09 05 01 09 88 01 04 00 00"
    ...
    # Format Type I: 2-ch, 2 bytes/sample, 16-bit, 48000 Hz
    "0B 24 02 01 02 02 10 01 80 BB 00"
    # Endpoint 0x82 IN: Isochronous Async, wMaxPacketSize 196 (0x00C4), bInterval 4
    "09 05 82 05 C4 00 04 00 00"
    # CS Audio Streaming Endpoint
    "07 25 01 00 00 00 00"
    # --- Interface 3: HID (DualSense gamepad) ---
    "09 04 03 00 02 03 00 00 00"
    # HID descriptor (bcdHID 1.11, report descriptor length 273 / 0x0111)
    "09 21 11 01 00 01 22 11 01"
    # Endpoint 0x84 IN: Interrupt, 64 bytes, bInterval 6
    "07 05 84 03 40 00 06"
    # Endpoint 0x03 OUT: Interrupt, 64 bytes, bInterval 6
    "07 05 03 03 40 00 06"
)
...
# Genuine 273-byte DualSense USB HID report descriptor captured from physical
# hardware.  Report ID 0x01 is unchanged, so DualSenseInputReport01 is unchanged.
...
DUALSENSE_STRING_PRODUCT = build_string_descriptor("Wireless Controller")
